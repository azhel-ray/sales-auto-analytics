...
> **Penting:** Vercel **tidak mendukung MySQL**. Schema Prisma sudah diganti ke **PostgreSQL**, jadi sebelum deploy pastikan database Postgres sudah siap (contoh gratis: [Neon](https://neon.tech)).

### 1. Siapkan Database PostgreSQL (Neon)

1. Daftar di [neon.tech](https://neon.tech) → buat project baru.
2. Copy **connection string**-nya (format: `postgresql://user:password@ep-xxx.region.aws.neon.tech/nama_db?sslmode=require`).

### 2. Push schema & seed (opsional)

```bash
# dari root project
npm install                                  # juga menjalankan prisma generate
npx prisma db push --schema backend/prisma/schema.prisma
node backend/prisma/seed.js                  # data awal + akun demo
```

Atau jalankan langsung dari Neon console/SQL editor menggunakan isi `backend/prisma/schema.prisma` (schema.sql tidak ada — Prisma memakai schema.prisma sebagai sumber kebenaran).

### 3. Deploy ke Vercel

1. Push project ini ke GitHub/GitLab.
2. Di [vercel.com/new](https://vercel.com/new), **Import** repository tersebut.
3. **Framework Preset:** pilih `Vite` (root otomatis terdeteksi dari `frontend/`).
4. Di **Environment Variables**, tambahkan:
   | Nama | Nilai |
   |---|---|
   | `DATABASE_URL` | Connection string PostgreSQL (dari Neon) |
   | `JWT_SECRET` | String acak panjang |
5. Klik **Deploy**. Vercel akan:
   - Membangun frontend (`frontend/dist`) sebagai static.
   - Membuat serverless function untuk Express API dari `api/index.js`.
   - Routing `/api/*` → API, sisanya → SPA (`index.html`).

Frontend memakai `VITE_API_URL` default `/api`, jadi tidak perlu diubah — API dan frontend berjalan di domain yang sama.

### 4. Verifikasi setelah deploy

- `https://<domain>/api/health` → `{"status":"OK",...}`
- Login di `https://<domain>/login` dengan akun seed:
  - **Owner:** `owner@geprek.com` / `admin123`
  - **Kasir:** `kasir@geprek.com` / `admin123`

### Catatan

- **File upload** tidak dipakai aplikasi ini (gambar produk hanya URL), jadi tidak ada masalah dengan fungsi serverless.
- **Dev lokal** sekarang juga memakai PostgreSQL — ubah `DATABASE_URL` di `backend/.env` sesuai Postgres lokal/Laragon atau gunakan URL Neon yang sama.
- Struktur deploy ada di `vercel.json` (pola `@vercel/node` + `@vercel/static-build`).
